...
recordRoutes.route("/record").get(function (req, res) {
    ...
    db_connect
        .collection("high_score").find({}).toArray(function (err, result) {
            if (err) throw err;
            res.json(result);
        });
});
...
recordRoutes.route("/record/:id").get(function (req, res) {
    ...
    db_connect
        .collection("high_score").findOne(myquery, function (err, result) {
            if (err) throw err;
            res.json(result);
        });
});
...
recordRoutes.route("/record/add").post(function (req, response) {
    ...
    db_connect.collection("high_score").insertOne(myobj, function (err, res) {
        if (err) throw err;
        response.json(res);
    });
});
...
recordRoutes.route("/update/:id").post(function (req, response) {
    ...
    recordRoutes.route("/:id").delete((req, response) => {
        let db_connect = dbo.getDb();
        let myquery = { _id: ObjectId(req.params.id) };
        db_connect.collection("high_score").deleteOne(myquery, function (err, obj) {
            if (err) throw err;
